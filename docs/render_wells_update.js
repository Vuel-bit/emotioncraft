...
(() => {
  ...
  function updateInteriorStack(view, dt, r, baseCol, spinRaw, i, tNow, opts) {
    if (!view || !view.interior || !view.pigment) return;

    const sv = spinVisual(spinRaw || 0);
    const dir = sv.dir;
    const spinNorm = sv.spinNorm;
    const magEff = sv.magEff;
    const omega = sv.omega;

    let omegaAbsEff = Math.abs(omega);
    if (!dir && opts && typeof opts.minOmegaAbs === 'number' && isFinite(opts.minOmegaAbs) && opts.minOmegaAbs > 0) {
      omegaAbsEff = Math.max(omegaAbsEff, opts.minOmegaAbs);
    }

    const omegaFx = omega * OMEGA_FX_MULT;
    const omegaFxForFx = (dir ? omegaFx : (omegaAbsEff * OMEGA_FX_MULT));

    const baseD = Math.max(2, r * 2);

    // Size core textures to the current radius (shared with wells).
    view.pigment.width = baseD;
    view.pigment.height = baseD;
    if (view.rippleA) { view.rippleA.width = baseD * 1.08; view.rippleA.height = baseD * 1.08; }
    if (view.rippleB) { view.rippleB.width = baseD * 1.18; view.rippleB.height = baseD * 1.18; }
    if (view.swirlA) { view.swirlA.width = baseD * 1.22; view.swirlA.height = baseD * 1.22; }
    if (view.swirlB) { view.swirlB.width = baseD * 1.35; view.swirlB.height = baseD * 1.35; }
    if (view.inkDark) { view.inkDark.width = baseD * 1.4; view.inkDark.height = baseD * 1.4; }
    if (view.inkLight) { view.inkLight.width = baseD * 1.25; view.inkLight.height = baseD * 1.25; }
    if (view.waveHand) { view.waveHand.width = baseD * 1.15; view.waveHand.height = baseD * 1.15; }
    if (view.marbleA) { view.marbleA.width = baseD * 1.05; view.marbleA.height = baseD * 1.05; }
    if (view.marbleB) { view.marbleB.width = baseD * 0.98; view.marbleB.height = baseD * 0.98; }
    if (view.tracers) { view.tracers.width = baseD * 1.12; view.tracers.height = baseD * 1.12; }
    if (view.highlight) { view.highlight.width = baseD * 1.06; view.highlight.height = baseD * 1.06; }
    if (view.sheen) { view.sheen.width = baseD * 1.2; view.sheen.height = baseD * 1.2; }
    if (view.coreGlow) { view.coreGlow.width = baseD * 0.72; view.coreGlow.height = baseD * 0.72; }
    if (view.edgeShade) { view.edgeShade.width = baseD * 1.02; view.edgeShade.height = baseD * 1.02; }

    // Tints must match the base hue exactly (avoid muting).
    try {
      if (typeof baseCol === 'number' && isFinite(baseCol)) {
        view.pigment.tint = baseCol;
        if (view.rippleA) view.rippleA.tint = baseCol;
        if (view.rippleB) view.rippleB.tint = baseCol;
        if (view.swirlA) view.swirlA.tint = baseCol;
        if (view.swirlB) view.swirlB.tint = baseCol;
        if (view.waveHand) view.waveHand.tint = baseCol;
        if (view.marbleA) view.marbleA.tint = baseCol;
        if (view.marbleB) view.marbleB.tint = baseCol;
        if (view.coreGlow) view.coreGlow.tint = baseCol;
      }
    } catch (_) {}

    // Animate using the same time-driven parameters as wells.
    if (!view._ripT) view._ripT = 0;
    const dtScale = 1 + omegaAbsEff * 1.25;
    view._ripT += dt * dtScale;

    if (!view._swirlAng) view._swirlAng = 0;
    if (!view._sheenAng) view._sheenAng = 0;
    view._swirlAng += omega * dt * 0.65;
    view._sheenAng += dt * (0.08 + omegaAbsEff * 0.22);

    const pWob = Math.sin(view._ripT * 0.55 + i * 1.3) * 0.035;
    const wob2 = Math.sin(view._ripT * 0.85 + i * 0.9) * 0.06;

    view.pigment.rotation = (spinRaw === 0 ? pWob : pWob * 0.4);

    // Ripples (keep direction only when dir != 0)
    const rippleBias = (spinNorm - 0.5) * 0.22;
    if (view.rippleA) {
      view.rippleA.rotation = view._ripT * 0.25 + dir * view._ripT * 0.35 * SPIN_SPEED_X + wob2;
      view.rippleA.alpha = 0.10 + 0.06 * magEff;
      view.rippleA.x = rippleBias * r;
      view.rippleA.y = -rippleBias * r * 0.6;
    }
    if (view.rippleB) {
      view.rippleB.rotation = -view._ripT * 0.18 + dir * -view._ripT * 0.28 * SPIN_SPEED_X - wob2 * 0.8;
      view.rippleB.alpha = 0.07 + 0.05 * magEff;
      view.rippleB.x = -rippleBias * r * 0.8;
      view.rippleB.y = rippleBias * r * 0.45;
    }

    // Swirls / ink
    if (view.swirlA) {
      view.swirlA.rotation = view._swirlAng * 0.65 + wob2;
      view.swirlA.alpha = 0.05 + 0.05 * magEff;
    }
    if (view.swirlB) {
      view.swirlB.rotation = -view._swirlAng * 0.45 - wob2 * 0.8;
      view.swirlB.alpha = 0.035 + 0.04 * magEff;
    }
    if (view.inkDark) {
      view.inkDark.rotation = view._ripT * 0.06 + wob2 * 0.25;
      view.inkDark.alpha = 0.05;
    }
    if (view.inkLight) {
      view.inkLight.rotation = -view._ripT * 0.05 - wob2 * 0.18;
      view.inkLight.alpha = 0.045;
    }
    if (view.waveHand) {
      view.waveHand.rotation = view._ripT * 0.12 + wob2 * 0.3;
      view.waveHand.alpha = 0.02 + 0.04 * magEff;
    }

    // Marble / tracers
    if (view.marbleA) {
      view.marbleA.rotation = view._ripT * 0.07 + wob2 * 0.15;
      view.marbleA.alpha = 0.06 + 0.06 * magEff;
    }
    if (view.marbleB) {
      view.marbleB.rotation = -view._ripT * 0.06 - wob2 * 0.12;
      view.marbleB.alpha = 0.05 + 0.05 * magEff;
    }
    if (view.tracers) {
      view.tracers.rotation = view._ripT * 0.09;
      view.tracers.alpha = 0.05 + 0.06 * magEff;
    }

    // Highlight / sheen (keep hue identity; no dark wash)
    if (view.highlight) {
      view.highlight.alpha = 0.08 + 0.08 * magEff;
      const hx = Math.cos(view._sheenAng) * r * 0.12;
      const hy = Math.sin(view._sheenAng * 1.1) * r * 0.10;
      view.highlight.position.set(hx, hy);
    }
    if (view.sheen) {
      view.sheen.alpha = 0.035 + 0.03 * magEff;
      const sx = Math.cos(-view._sheenAng * 0.8) * r * 0.16;
      const sy = Math.sin(-view._sheenAng * 0.9) * r * 0.14;
      view.sheen.position.set(sx, sy);
    }

    if (view.coreGlow) {
      view.coreGlow.alpha = 0.05 + 0.10 * magEff;
    }
    if (view.edgeShade) {
      view.edgeShade.alpha = 0.085; // subtle inner depth; avoid muting
    }

    // Update the same nebula/water FX used by wells (visual-only)
    try {
      if (EC.RENDER_WELLS_FX && EC.RENDER_WELLS_FX.updateNebulaFX) {
        // For dir==0, feed a tiny omegaFx magnitude so the fluid doesn't look inert.
        const magForFx = Math.max(magEff, Math.min(1, omegaAbsEff * 0.25));
        EC.RENDER_WELLS_FX.updateNebulaFX(view, dt, r, baseCol, dir, magForFx, spinNorm, omegaFxForFx, (tNow || 0), (view._ripT || 0), i);
      }
    } catch (_) {}
  }
  ...
  // Expose reusable helper
  EC.RENDER_WELLS_UPDATE = EC.RENDER_WELLS_UPDATE || {};
  EC.RENDER_WELLS_UPDATE.updateInteriorStack = updateInteriorStack;
})();
