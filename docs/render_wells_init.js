...
(() => {
  ...
  // PASS A40: helper to build a well interior stack (visual-only) that can be reused
  // by other views (e.g., psyche wedges) while keeping the exact same textures + FX.
  // Returns a lightweight "vw" object with the same sprite fields the well updater expects.
  function makeWellInteriorStack(tintColor, opts) {
    ensureTextures();
    ...
    // Pigment base
    const pigment = new Sprite(TEX.body);
    pigment.anchor.set(0.5);
    pigment.tint = tintColor;
    pigment.alpha = 1.0;
    pigment.blendMode = BLEND.NORMAL;
    pigment.name = 'pigment';

    // Ripples / caustics
    const rippleA = new Sprite(TEX.rippleCircle);
    rippleA.anchor.set(0.5);
    rippleA.tint = tintColor;
    rippleA.alpha = 0.12;
    rippleA.blendMode = BLEND.ADD;
    rippleA.name = 'rippleA';

    const rippleB = new Sprite(TEX.rippleCircle);
    rippleB.anchor.set(0.5);
    rippleB.tint = tintColor;
    rippleB.alpha = 0.08;
    rippleB.blendMode = BLEND.ADD;
    rippleB.name = 'rippleB';

    // Swirl / ink / hand-wave
    const swirlA = new Sprite(TEX.swirl);
    swirlA.anchor.set(0.5);
    swirlA.tint = tintColor;
    swirlA.alpha = 0.06;
    swirlA.blendMode = BLEND.SCREEN;
    swirlA.name = 'swirlA';

    const swirlB = new Sprite(TEX.swirl);
    swirlB.anchor.set(0.5);
    swirlB.tint = tintColor;
    swirlB.alpha = 0.04;
    swirlB.blendMode = BLEND.SCREEN;
    swirlB.name = 'swirlB';

    const inkDark = new Sprite(TEX.noise);
    inkDark.anchor.set(0.5);
    inkDark.tint = 0x000000;
    inkDark.alpha = 0.055;
    inkDark.blendMode = BLEND.MULTIPLY;
    inkDark.name = 'inkDark';

    const inkLight = new Sprite(TEX.noise);
    inkLight.anchor.set(0.5);
    inkLight.tint = 0xffffff;
    inkLight.alpha = 0.05;
    inkLight.blendMode = BLEND.SCREEN;
    inkLight.name = 'inkLight';

    const waveHand = new Sprite(TEX.hand);
    waveHand.anchor.set(0.5);
    waveHand.tint = tintColor;
    waveHand.alpha = 0.03;
    waveHand.blendMode = BLEND.ADD;
    waveHand.name = 'waveHand';

    // Marble / tracers
    const marbleA = new Sprite(TEX.marble);
    marbleA.anchor.set(0.5);
    marbleA.tint = tintColor;
    marbleA.alpha = 0.08;
    marbleA.blendMode = BLEND.SCREEN;
    marbleA.name = 'marbleA';

    const marbleB = new Sprite(TEX.marble);
    marbleB.anchor.set(0.5);
    marbleB.tint = tintColor;
    marbleB.alpha = 0.06;
    marbleB.blendMode = BLEND.SCREEN;
    marbleB.name = 'marbleB';

    const tracers = new Sprite(TEX.tracers);
    tracers.anchor.set(0.5);
    tracers.tint = 0xffffff;
    tracers.alpha = 0.06;
    tracers.blendMode = BLEND.ADD;
    tracers.name = 'tracers';

    // Highlights / sheen
    const highlight = new Sprite(TEX.highlight);
    highlight.anchor.set(0.5);
    highlight.tint = 0xffffff;
    highlight.alpha = 0.10;
    highlight.blendMode = BLEND.ADD;
    highlight.name = 'highlight';

    const sheen = new Sprite(TEX.highlight);
    sheen.anchor.set(0.5);
    sheen.tint = 0xffffff;
    sheen.alpha = 0.045;
    sheen.blendMode = BLEND.SCREEN;
    sheen.name = 'sheen';

    // Core + edge shade
    const coreGlow = new Sprite(TEX.circle);
    coreGlow.anchor.set(0.5);
    coreGlow.tint = tintColor;
    coreGlow.alpha = 0.06;
    coreGlow.blendMode = BLEND.SCREEN;
    coreGlow.name = 'coreGlow';

    const edgeShade = new Sprite(TEX.edge);
    edgeShade.anchor.set(0.5);
    edgeShade.tint = 0x000000;
    edgeShade.alpha = 0.10;
    edgeShade.blendMode = BLEND.MULTIPLY;
    ...
    // Apply the exact same interior FX stack as wells (visual-only).
    try {
      if (EC.RENDER_WELLS_FX && EC.RENDER_WELLS_FX.applyNebulaFX) {
        EC.RENDER_WELLS_FX.applyNebulaFX(vw, { alt: !!(opts && opts.alt) });
      }
    } catch (e) {}
  ...
  EC.RENDER_WELLS_INIT.makeWellInteriorStack = makeWellInteriorStack;
